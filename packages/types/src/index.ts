...
// ── Journal ───────────────────────────────────────────────────────────────────
//
// Journal's domain shapes (JournalEntryProperties, JournalSynthesisProperties,
// the source/kind vocabulary) moved to `@nau/journal` (nau#96) — a package
// with no dependency on this one, so the same rules can validate a capture on
// a device with no server relationship. This file keeps only the wire DTO for
// asking Journal to generate a synthesis, since @nau/types is where cross-
// package request shapes live; the domain types it references are inlined as
// literals rather than imported, so the dependency stays one-way
// (@nau/journal has no reason to know this package exists).
