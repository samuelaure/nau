...
describe('ScheduleService', () => {
  ...
  it('should upsert a schedule', async () => {
    const date = new Date();
    prisma.schedule.upsert.mockResolvedValueOnce({ id: 'sch-1', blockId: 'b1', startDate: date, endDate: null, rrule: null, completedAt: null, createdAt: date, updatedAt: date } as Schedule);
    
    await service.upsert('b1', date);
    
    expect(prisma.schedule.upsert).toHaveBeenCalledWith({
      where: { blockId: 'b1' },
      create: { blockId: 'b1', startDate: date, endDate: undefined, rrule: undefined },
