...
export const PUT = auth(async function PUT(req, props) {
    ...
    try {
        ...
        // 1. Better Compilation Logic using AI
        // We will ask the model to analyze the JSON structure and output a strict Prompt definition matrix.
        ...
        // 2. Save everything precisely
        const updated = await (prisma.template as any).update({
            where: { id },
            data: {
                schemaJson,
                systemPrompt: generatedSystemPrompt // Saves the compiled parameters correctly
            }
        })
