  ...
  return (
    <div className="animate-fade-in">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-heading font-semibold mb-2">{workspace.name} Overview</h1>
          <p className="text-text-secondary">
            Manage the Instagram profiles linked to this workspace.
          </p>
        </div>
        <AddAccountButton
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-6">
        {accounts.map((account: AccountWithCounts) => (
          ...
          >
            ...
            <Link
              href={`/dashboard/workspace/${workspaceId}/account/${account.id}`}
        ...
        {accounts.length === 0 && (
          <Card className="col-span-full py-20 px-10 text-center border-dashed bg-transparent flex flex-col items-center">
            ...
            <h3 className="text-xl font-semibold mb-2">No components linked</h3>
            <p className="text-text-secondary mb-6">
              Connect an Instagram Business account to start publishing.
            </p>
            <AddAccountButton workspaceId={workspaceId} />
          </Card>
        )}
      </div>
    </div>
