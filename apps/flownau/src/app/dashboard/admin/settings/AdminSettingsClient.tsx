...
const FEATURES: { key: AdminModelFeature; label: string; description: string }[] = [
  {
    key: 'ideation',
    label: 'Ideation',
    description: 'Content idea generation — topic → concept ideas with angles.',
  },
  {
    key: 'drafting',
    label: 'Drafting',
    description: 'Script / creative draft generation — idea → video script + caption.',
  },
  {
    key: 'planning',
    label: 'Planning',
    description: 'Calendar strategist — orders approved content by posting priority.',
  },
]
...
export default function AdminSettingsClient({ initialSettings }: Props) {
  ...
  async function handleSave() {
    ...
    for (const feature of FEATURES) {
      body[ADMIN_MODEL_SETTING_KEYS[feature.key]] =
        values[ADMIN_MODEL_SETTING_KEYS[feature.key]] ?? ''
    }
    ...
    <div className="max-w-2xl mx-auto px-6 py-10 space-y-10">
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">
          Platform-wide model configuration. Changes apply immediately to all users.
        </p>
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-zinc-500 mb-4">
          Overrides the default model per pipeline stage. "Platform default" uses the built-in
          feature mapping.
        </p>
