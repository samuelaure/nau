...
- **Role:** Flownaŭ's *Automatic* Idea Generation leverages Nauthenticity's curated content concepts (`InspoItems`).
- **Data Flow:**
...
- **Fallback Logic:** If Nauthenticity is down (e.g. `ECONNREFUSED` on `nau-network`), the IdeationEngine degrades gracefully to only use internal *Brand DNA* context for generating standard thematic ideas.
